...
- `Component` — classe base astratta con lifecycle `awake()` → `start()` → `update(dt)` → `fixedUpdate(dt)` (usato dalla fisica in Fase 3) → `onDestroy()`
...
- `OrbitCameraController` — wrapper su `OrbitControls` (three.js addon), API `update(dt)` / `setTarget(x, y, z)` / `dispose()`, più `get/set enabled` (Fase 4C, usato dall'editor per disattivare l'orbit durante il drag del gizmo).
...
`@engine/core` espone inoltre, per la fisica (`@dimforge/rapier3d-compat`):

- `Physics` — oggetto raggruppato: `initPhysics()` (async, da chiamare prima di qualunque altro uso), `step()`, `raycast(...)`, `setGravity(...)`.
- `RigidBody` (component) — `RigidBodyType.Dynamic | Kinematic | Fixed`.
- `Collider` (component astratto) — `BoxCollider` / `SphereCollider`.

La creazione del corpo/collider Rapier è deferita al primo tick fisico successivo ad `addComponent`, non sincrona in `awake()`: rende l'ordine `RigidBody`/`Collider` su un GameObject irrilevante (Unity-style).

## Editor (Fase 4)

`packages/editor` — shell React 18 + Vite, tre pannelli (Hierarchy | Viewport | Inspector), gizmo di trasformazione, PWA installabile. Vedi [`packages/editor/README.md`](./packages/editor/README.md) per dettagli e istruzioni d'uso.
